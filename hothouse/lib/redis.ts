...
const REDIS_SERVERS = parseUris(process.env.REDIS_SERVERS || '')
const REDIS_ENABLE_TLS = process.env.REDIS_ENABLE_TLS === 'true'

if (!process.env.REDIS_SERVERS || !REDIS_SERVERS.length) {
    throw new Error('REDIS_SERVERS is not set. You can create a .env file with the servers.')
}

console.log({ REDIS_SERVERS })
...
function redis(target: keyof typeof redises, forceNew = false): Redis {
    ...
    if (redises[target].handle && !forceNew) return redises[target].handle
        ...
        handle.on('error', err => {
            if (
                (err.message === 'Connection is closed.' || err.code === 'ECONNREFUSED') &&
        ...
        handle.waitForReady = function () {
            return new Promise((resolve, reject) => {
                ...
                else if (handle.status !== 'connecting' && handle.status !== 'connect') {
                    handle.connect().catch(err => {
                        if (handle.status === 'reconnecting') {
                            return
                        }
                        logger.error('Failed to connect to redis:', { err, status: handle.status })
                        reject(err)
                    })
                }
            })
